...
generate_sales_type <- function(n) {
  sales_types <- c(rep("Outgoing", round(n * 0.7)), rep("Ingoing", round(n * 0.3)))
...
generate_amounts <- function(n, sales_type, purposes, payers) {
  ...
  for (i in 1:n) {
    if (sales_type[i] == "Ingoing") {
      if (payers[i] == "Employer") {
        amounts[i] <- runif(1, min = 4000, max = 5000)
      } else {
      ...
      if (purposes[i] == "Groceries") {
        ...
        amounts[i] <- runif(1, min = 20, max = 400)
      } else if (purposes[i] == "Banking") {
        amounts[i] <- runif(1, min = 100, max = 5000)
      } else if (purposes[i] == "Personal") {
        amounts[i] <- runif(1, min = 10, max = 500)
      } else if (purposes[i] == "Travel") {
        amounts[i] <- runif(1, min = 50, max = 2000)
      } else if (purposes[i] == "Healthcare") {
        amounts[i] <- runif(1, min = 10, max = 100)
      } else if (purposes[i] == "Dining") {
        amounts[i] <- runif(1, min = 10, max = 360)
      } else {
        amounts[i] <- runif(1, min = 1, max = 10000)
      }
    }
  }
...
generate_payers_and_payees <- function(n, sales_type, names) {
  payers <- ifelse(sales_type == "Outgoing", "Lennard Riede", sample(c("Arsenii Vesnovskyi", "Marta Peregrin", "Simon Han", "Employer"), n, replace = TRUE))
  payees <- ifelse(sales_type == "Outgoing", names, "Lennard Riede")
  list(payers = payers, payees = payees)
}

generate_salary_transactions <- function(start_date, end_date, amount = 4000) {
  dates <- seq.Date(as.Date(start_date), as.Date(end_date), by = "month")
  n <- length(dates)
  payers <- rep("Employer", n)
  payees <- rep("Lennard Riede", n)
  purposes <- rep("Salary", n)
...
generate_rent_transactions <- function(start_date, end_date, amount = -1500) {
  dates <- seq.Date(as.Date(start_date), as.Date(end_date), by = "month")
  n <- length(dates)
  payers <- rep("Lennard Riede", n)
...
# Generate unique IBANs for each payer and payee, including Employer and Landlord
unique_entities <- unique(c(payers_and_payees$payers, payers_and_payees$payees, "Employer", "Landlord"))
...
salary_transactions <- generate_salary_transactions(start_date, end_date)
salary_transactions$Payer_IBAN <- ibans["Employer"]
salary_transactions$Payee_IBAN <- ibans["Lennard Riede"]

# Generate rent transactions
rent_transactions <- generate_rent_transactions(start_date, end_date)
rent_transactions$Payer_IBAN <- ibans["Lennard Riede"]
rent_transactions$Payee_IBAN <- ibans["Landlord"]

# Append salary and rent transactions to the synthetic data
synthetic_data <- bind_rows(synthetic_data, salary_transactions, rent_transactions)

# Calculate final balance starting from the specified budget
final_balance <- starting_budget + sum(synthetic_data$Amount)
cat("Final Balance:", final_balance, "\n")


# Export the generated data to a CSV file
write.csv(synthetic_data, "synthetic_data_worker.csv", row.names = FALSE)